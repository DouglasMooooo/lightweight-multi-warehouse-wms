...
export async function GET(request: Request) {
  ...
  const result = await timed(
    { route: "GET /api/transactions", queryName: "transactionPage", rowCount: (value) => value.rows.length },
    () => new PageQueryService().transactions({
      ...
      sku: q.get("sku") ?? undefined, sn: q.get("sn") ?? undefined,
    }),
  );
