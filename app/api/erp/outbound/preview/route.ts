...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await timed(
      { route: "POST /api/erp/outbound/preview", queryName: "erpOutboundPreview" },
      () => new WmsApplicationService().previewOutboundImport(shNo),
    ));
  } catch (error) {
    const status = error instanceof DomainError || error instanceof z.ZodError ? 400 : 502;
    return NextResponse.json({
      error: error instanceof Error ? error.message : "ERP preview failed.",
