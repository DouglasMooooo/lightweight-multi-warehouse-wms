...
| POST | `/api/outbound/{orderId}/preparation` | Validate or atomic mutation by `action` | Normal outbound preparation workspace. `validate`/`lookup` are read-only; `confirm` atomically records location, exact quantity, required SN assignment, Frozen, ledger, audit and readiness | line ID, location code, exact quantity, scan/paste/upload rows | row validation or confirmation with order status and Pickup Code | validation/domain code, `OUTBOUND_PREPARATION_FAILED` |
...
| POST | `/api/outbound/{orderId}/serials/commit` | Legacy exception recovery only; revalidate and bind exact SNs to an older prepared allocation | order line and selected SNs | assignment result plus re-evaluated order status | `INVALID_BATCH`, allocation/SN/domain codes |
