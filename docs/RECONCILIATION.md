...
For each serial-tracked Product balance, Physical Qty is compared with SNs at the same product, warehouse, location and condition. In_Stock, Prepared, Repair and Scrapped count as physically present; Outbound and In_Transit do not. Scrapped is still non-allocatable. Results are `SERIAL_COUNT_MATCH`, `SERIAL_COUNT_SHORTAGE` or `SERIAL_COUNT_EXCESS`.
...
The `/reconciliation` page shows severity counts and filters by issue type or SKU/location/SN/classification, and exports the filtered rows as CSV. Sources distinguish workbook current view, ledger projection, WMS, master data and workflow. Cached spreadsheet formula results may be read as data; formula expressions are never executed.
