...
- The final fixture SN prefix is `DEMO-TRANSFER-*`. Earlier `DEMO-TF-*` synthetic identities are retired only when every old SN is still untouched in the dedicated demo location. The controlled script appends paired Adjustment ledger evidence and AuditLog evidence while leaving net Physical Qty unchanged; it never rewrites the original Opening rows.
